...
public class HelloServerStreamingServerTest {
        ...
        public void test2() throws IOException {
                ...
                Collection<HelloReply> features = new ArrayList<>();
                features.add(
                                HelloReply.newBuilder().setMessage(" world").build());
                ...
                Iterator<HelloReply> reply = stub
                                .sayHelloServerStreaming(HelloRequest.newBuilder().setName("tmp").build());
                Collection<HelloReply> resArray = new ArrayList<>();
                while (reply.hasNext()) {
                        resArray.add(reply.next());
                }

                // Generate answer arraylist.
                Collection<HelloReply> expectedArray = new ArrayList<>();
                expectedArray.add(
                                HelloReply.newBuilder().setMessage("tmp world").build());

                assertEquals(expectedArray, resArray);
